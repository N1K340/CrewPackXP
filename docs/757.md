...
---
## Aircraft Settings
The aircraft settings window can be accessed via three methods.
...
3. Assigning a key command _____
...
> 13. Volume slider to control base level for the flight attendant making PAs in the cabin.

The PA volume is dependent upon the position of the cockpit door in the 757 / 767. When the door is open, the PA volume will be increased by 50%. The slider in settings represents the volume setting for when the cockpit door is closed.  
=======
The aircraft settings window can be accessed via three methods.  
1. Hidden click spot in the title of the Status HUD  
2. Via the X-Plane menu bar system by navigating:  
...
```
The `intHudXStart` variable controls the position of the HUD horizontally on the screen in pixels.  
`0` is the far left of the screen.  

The `intHudYStart` variable controls the position of the HUD vertically on the screen in pixels.  
...
### FO Pre-Flight
The First Officer pre-flight scan flow can be called from the either the Status HUD, or automatically via the aircraft settings.
...
> - Set the pressurization panel to current altitude;
