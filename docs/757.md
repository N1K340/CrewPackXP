...
> 5. The First Officer will automatically perform the pre-flight scan flow once the script has been initialised.  
...
The First Officer secure checks can only be called from the Status HUD. The First Officer will only complete these actaions provided the beacon is off, and both engines N2 is below 5%. Whilst running the checks, there will be a moment where the First Officer will pause and await the APU to shut down prior to completely powering down the aircraft.
